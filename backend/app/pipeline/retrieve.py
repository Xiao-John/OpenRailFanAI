...
_TIME_WINDOWS = (
    # 「饭前/饭后」这类相对钟点放在最前面：它比"中午/晚上"更具体
    (("午饭后", "午饭之后", "中饭后"), "12:00", ""),
    (("晚饭后", "晚饭之后"), "18:00", ""),
    (("午饭前", "午饭之前", "中饭前"), "", "12:00"),
    (("晚饭前", "晚饭之前"), "", "18:00"),
...
# 值必须与 12306 的座位键名对得上（工具侧按**子串**匹配）：
#   business / first_class / second_class / advanced_soft_sleeper /
#   soft_sleeper / hard_sleeper / soft_seat / hard_seat / no_seat / dongwo
# 顺序敏感：「高级软卧」必须排在「软卧」前面，否则会被子串匹配抢先命中。
_SEAT_KEYWORDS = (
    (("商务座", "商务"), "business"),
    (("一等座",), "first_class"),
    (("二等座",), "second_class"),
    (("高级软卧", "高包"), "advanced_soft_sleeper"),
    (("动卧",), "dongwo"),
    # 泛称"卧铺"不指定软硬：用 "sleeper" 去子串匹配 soft_sleeper/hard_sleeper/动卧
    # 都能命中，正好是用户的意思（别硬猜成软卧或硬卧）
    (("卧铺", "睡觉"), "sleeper"),
    (("软卧",), "soft_sleeper"),
    (("硬卧",), "hard_sleeper"),
    (("软座",), "soft_seat"),
...
# 问"经停/历时/站序/用时"时才允许下发图定参考时刻（D06 红线修复的检索侧开关）
# 2026-09-17 补：用户实测常问「用时多久/耗时/坐多久」，旧正则只认"要多久/多长时间"，
# 导致这类"已发车次的用时"问题拿不到图定表 → 只能答"数据不可得"。
_STOPS_QUESTION_RE = re.compile(
    r"(经停|停靠|经过哪些站|途经|站序|全程|历时|多长时间|要多久|几个小时"
    r"|用时|耗时|多久|坐多久)"
)
...
def _parse_time_window(*texts: str | None) -> tuple[str, str]:
    ...
    为什么要有"具体钟点"这一支：探针实测「8点以后」「10点前」是极常见的说法，
    而原先只认"早上/下午/晚上"这类**粗时段**，具体钟点一律被静默忽略 ——
    筛选没生效，但模型并不知道（本项目的"静默失败"老问题）。
    粗时段词与钟点同现时按粗时段修正（"下午3点以后" = 15:00）。
    """
    blob = " ".join(t for t in texts if t)

    m = re.search(r"(\d{1,2})\s*[点:：时]\s*(以后|之后|起|以前|之前|后|前)", blob)
    if m:
        hour = int(m.group(1))
        if hour <= 12 and any(w in blob for w in ("下午", "午后", "晚上", "晚间",
                                                  "夜里", "夜间", "傍晚")):
            if hour < 12:
                hour += 12                      # 下午3点 → 15 点
        elif hour == 12 and any(w in blob for w in ("上午", "早上", "早晨", "凌晨")):
            hour = 0                            # 上午12点 → 0 点
        if 0 <= hour <= 23:
            hh = f"{hour:02d}:00"
            if m.group(2) in ("以后", "之后", "后", "起"):
                return hh, ""
            return "", hh
