...
TIME_CASES: list[tuple[str | None, str | None]] = [
    ...
    ("十月", None),                 # 只有月
    # 公历固定的节日简称（f20f2b4 之后补的）
    ("十一", _md(10, 1)),
    ("国庆", _md(10, 1)),
    ("五一", _md(5, 1)),
    ("元旦", _md(1, 1)),
    ("09-28", _md(9, 28)),          # 裸 MM-DD（无年份）
    # 农历节日**有意不支持**：逐年变动，不是常量表能表达的。钉住"不猜"
    ("春节", None),
    ("中秋", None),
    # 红线：「长假」是**假期段**而非某一天，不能解析成具体日期
    ("十一长假", None),
    ("国庆假期", None),
]
...
MIN_CASES = {"time": 40, "window": 20, "seat": 19, "type": 17}
...
def main():
    ...
    test_type_phrasings()
    print("\n说法语料探针全部通过 ✔")
    print("已知缺口：农历节日（春节/清明/端午/中秋）有意不支持 —— 逐年变动，需农历换算表，")
    print("  不猜；工具层会如实说明「没听懂，已按今天处理」。公历固定的节日（元旦/五一/国庆）已支持。")
